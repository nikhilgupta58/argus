...
export async function verifyBundle(tarPath: string): Promise<BundleManifest> {
  ...
  try {
    ...
    const valid = ed25519.verify(hexToBytes(signature), payloadHash, hexToBytes(raw.publisherIdentity.publicKeyHex));
    if (!valid) throw new Error("Bundle signature verification failed — bundle may have been tampered with");
