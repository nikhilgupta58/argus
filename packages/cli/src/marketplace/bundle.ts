...
export async function createBundle(opts: CreateBundleOptions): Promise<BundleManifest> {
  const { sourceDir, name, version, contractKinds, publisherIdentity, privateKey, outputPath } =
    opts;

  const specPath = join(sourceDir, "specialist.ts");
  const codeBytes = readFileSync(specPath);
  const codeHash = bytesToHex(
    blake3(new Uint8Array(codeBytes.buffer, codeBytes.byteOffset, codeBytes.byteLength)),
  );
