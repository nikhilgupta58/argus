...
import type { KeyPair } from "@argus/lineage";
...
const publishCmd = new Command("publish")
  ...
  .option(
    "--passphrase <pass>",
    "Passphrase to decrypt the publisher key (use env ARGUS_PASSPHRASE)",
  )
  .option("--out <dir>", "Output directory for the bundle (default: current directory)", ".")
  .action(
    async (specPath: string, opts: { publisher: string; passphrase?: string; out: string }) => {
      ...
      if (
        ...
        !Array.isArray(mod.default.contractKinds)
      ) {
        console.error(
          pc.red("specialist.ts default export must have name, version, and contractKinds"),
        );
        process.exit(1);
      }
