...
const registerCmd = new Command("register")
  ...
  .action((opts: { name: string; passphrase?: string }) => {
    ...
    try {
      store.register({ id, name: opts.name, public_key_hex: hex.publicKey, created_at: new Date().toISOString() });
    } finally {
      store.close();
    }
...
const listCmd = new Command("list")
  ...
  .action(() => {
    ...
    for (const p of publishers) {
      console.log(`${pc.green(p.name)}  ${pc.dim(p.id)}  pubkey: ${p.public_key_hex.slice(0, 16)}...`);
    }
  });
